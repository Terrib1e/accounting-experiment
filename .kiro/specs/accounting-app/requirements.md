...
### Requirement 33: Client Portal

**User Story:** As a client of the accounting firm, I want a secure portal, so that I can view my financials, pay invoices, and share documents.

#### Acceptance Criteria

1. WHEN a client logs in THEN THE System SHALL provide a dedicated dashboard showing outstanding invoices and active jobs
2. THE System SHALL allow clients to view and download their invoices
3. THE System SHALL allow clients to securely upload documents (e.g., tax forms, receipts) to a shared vault
4. THE System SHALL allow clients to view documents shared by the firm
5. THE System SHALL allow clients to make payments for outstanding invoices via credit card or ACH
6. THE System SHALL allow clients to view the status of active jobs (e.g., "Tax Return 2024 - In Progress")
7. THE System SHALL enforce strict isolation so clients can ONLY access their own data

### Requirement 34: Practice Workflow (Jobs)

**User Story:** As an accountant, I want to manage client work as "Jobs", so that I can track deadlines and staff workload.

#### Acceptance Criteria

1. WHEN a user creates a Job THEN THE System SHALL link it to a specific Client (Contact)
2. THE System SHALL support different Job Types (e.g., Tax Return, Monthly Bookkeeping, Payroll) with custom workflows
3. THE System SHALL allow assigning Jobs and specific Tasks to staff members
4. THE System SHALL track the status of Jobs (e.g., PLANNED, IN_PROGRESS, WAITING_FOR_CLIENT, REVIEW, COMPLETED)
5. WHEN a Job is due THEN THE System SHALL notify the assigned staff member
6. THE System SHALL allow linking Documents and Invoices to specific Jobs
7. THE System SHALL provide a Kanban-style board to view all active Jobs
